...
### Нерешённое (следующие шаги)

- **Карточка ошибки по ссылке**: `POST /ui/entity/read` с телом
  `{"entities": [{"reference": {<typeref>}, "versionStamp": null}]}` даёт
  500 и на ошибке, и на проекте (варианты без stamp и с нулевым UUID
  проверены) — формат запроса ещё не подобран, копать классы `v`/`M` и
  сериализацию POJO в чанке 205.js (возможно, нужен дискриминатор типа
  или другой формат ссылки-ключа).
- **Номер ошибки**: подтверждено, что числа в адресах bugboard — тот же
  формат, что EF-номера releases: `EF_00_00928768` (releases) =
  `er-00-00928768` (bugboard). Рабочий способ получить ссылку ошибки:
  `ПолучитьОшибкуПоФрагментуСсылкиИзURL("<полная строка состояния>")`,
  например `prj-ssl22-er-00-00928768` → ссылка `Ошибки.Reference`.
  Поиск `НайтиОшибкиПоКоду("00-00928768")` возвращает Undefined (метод,
  видимо, ищет по внутреннему коду, не по публичному номеру).
- **`ВыполнитьПоискПоОшибкам`** отвечает пустым массивом — подобрать
  формат строки или контекст.
