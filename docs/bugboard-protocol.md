...
- **Карточка ошибки по ссылке**: `POST /ui/entity/read` с телом
  `{"entities": [{"reference": {<typeref>}}]}` даёт 500 (нужен точный
  формат — см. классы `v`/`M` в чанке 205.js: поля `reference`,
  `versionStamp`). `ПолучитьОшибкуПоФрагментуСсылкиИзURL` ждёт публичный
  номер ошибки (формат `NNN-NNNNNNNN`), UUID не принимает.
- **Соответствие EF-номеров (releases) ↔ номера bugboard**: коды
  `EF_00_00931848` / `00931848` методом `НайтиОшибкиПоКоду` не
  находятся — у bugboard своя нумерация; связь, вероятно, через поля
  карточки ошибки (после решения entity/read).
- **`ВыполнитьПоискПоОшибкам`** отвечает пустым массивом — подобрать
  формат строки или контекст (возможно, поиск по подпискам/доступным
  проектам после инициализации пользователя).
